...
这就是Go中错误处理机制的全部内容。该语言强制程序员在返回类型为[T0]的值的每个函数调用上始终测试错误状态。对于某些人来说，`if…not…nil error`处理习惯用法可能显得过于冗长，尤其是如果您来自一种具有正式异常机制的语言。然而，这里的好处是，程序可以构造一个健壮的执行流，程序员总是知道错误可能来自哪里，并适当地处理它们。
