...
本章对Go写作测试的实践进行了广泛的介绍。它讨论了几个关键主题，包括使用`go test`工具编译和执行自动化测试。读者学习了如何编写测试函数，以确保他们的代码得到正确的测试和覆盖。本章还讨论了测试 HTTP 客户端和服务器的主题。最后，本章介绍了基准测试的主题，作为使用内置 Go 工具自动化、分析和度量代码性能的一种方法。