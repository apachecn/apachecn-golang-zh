...
共享内存是线程之间进行通信的传统方式。Go 具有内置的同步功能，允许单个 goroutine 拥有共享数据。这意味着其他 goroutine 必须向拥有共享数据的单个 goroutine 发送消息，从而防止数据损坏！这样的 goroutine 称为**监视器 goroutine**。在围棋术语中，这是通过交流而不是通过共享进行的*共享。*
