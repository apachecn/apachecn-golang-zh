...
![](assets/d64b1112-ed81-4cf4-bd8f-0395d33baa5e.png)

单击“查看默认值”选项以创建具有默认设置的池：

![](assets/9a950ec8-a55f-42ae-80fe-bb7c1341db96.png)

单击导航窗格中的属性，并勾选电子邮件地址或电话号码下的允许电子邮件地址选项，以允许用户使用电子邮件地址登录：

![](assets/61b2d18f-bac9-4141-93d8-4598440cedaf.png)

返回查看并单击创建池。创建过程结束时应显示一条成功消息：

![](assets/8d7b4995-faf5-43ad-b16b-5adad82721cf.png)
...
![](assets/c7345bb0-aef1-47ca-b498-00796ba6a459.png)

选择创建应用程序客户端注册应用程序，并将**应用程序客户端 id**复制到剪贴板：

![](assets/8ae8223e-3303-4afc-abea-a50732e92be3.png)
...
![](assets/2bd40386-76ce-436d-9635-01efe30a58a9.png)

单击 createnewauthorizer 按钮并选择 Cognito。然后，选择我们之前创建的用户池，并将令牌源字段设置为`Authorization`。这定义了包含 API 调用方的身份令牌的传入请求头的名称`Authorization`：

![](assets/4019bf3f-c338-4cd7-8755-673abdce6987.png)

填写表单后，单击 Create 将 Cognito 用户池与 API 网关集成：

![](assets/dcd77acd-3df6-4ac7-9fab-0ce42b730c35.png)

您现在可以保护所有端点，例如，为了保护负责列出所有`movies`的端点。点击`/movies`资源下对应的`GET`方法：

![](assets/0e5002c8-6700-4773-9105-3e873cca9342.png)

单击方法请求框，然后单击授权，并选择我们之前创建的用户池：

![](assets/76a3b295-45d7-44dd-ae21-d01da1d28f3c.png)

将 OAuth Scopes 选项保留为`None`，并对其余方法重复上述步骤以保护它们：

![](assets/8a5148dc-b017-4b4e-b5ad-bc63e7b3672c.png)

完成后，重新部署 API 并将浏览器指向 API 网关调用 URL：

![](assets/ec588566-1148-4fab-bd68-c938c9ed5710.png)

这一次，端点是安全的，需要身份验证。您可以通过检查我们以前构建的前端来确认该行为。如果检查网络请求，API 网关请求应返回 401 未经授权的错误：

![](assets/8dbb072f-7e1b-4e50-b8e5-877f150e006b.png)

为了修复此错误，我们需要更新客户端（web 应用程序）以执行以下操作：
...
![](assets/6166182e-436c-4ee7-a251-0701a396d16b.png)

设置用户名和密码。如果要接收确认电子邮件，可以取消选中“将电子邮件标记为已验证”框：

![](assets/d936b90b-04b4-46ab-a506-65b1bba95a25.png)
...
![](assets/b30d2818-cda2-4dd0-8d2c-51b41b1adf88.png)
...
![](assets/bfb6749b-38ca-4334-84f9-4feacb9576ac.png)
...
![](assets/b2e88681-4a54-4ac7-b555-14d0cfb75711.png)
...
![](assets/f4cceb41-12f3-4215-bcb2-aa454bc017fa.png)

到目前为止，一切顺利。但是，数据库凭据是纯文本的！

![](assets/d42646b1-597a-4f6c-8e0d-26919861d601.png)
...
![](assets/bc5c3a85-f548-4bfa-9d00-378a07d81c00.png)

通过导航到 Identity and Access Management Console，您可以更改密钥并使用自己的密钥。然后，单击加密密钥：

![](assets/0ef2e744-13c3-4ff3-bb73-476fbb48bfdc.png)

单击“创建密钥”按钮创建新的客户主密钥：

![](assets/563e4e62-d659-444a-9648-a648d6647c4a.png)

通过**密钥管理服务**（**KMS**API）选择 IAM 角色和帐户来管理密钥。然后，选择创建 Lambda 函数时使用的 IAM 角色。这允许 Lambda 功能使用**客户主密钥**（**CMK**）并成功请求`encrypt`和`decrypt`方法：

![](assets/511f7fad-6191-4a7e-b796-0d12736c6f52.png)

创建密钥后，返回 Lambda 函数配置页面，将密钥更改为您刚刚创建的密钥：

![](assets/cd032e44-5ae4-4b8d-afc8-95a89d0c8dc6.png)
...
![](assets/825da2fe-8377-41e5-9e31-c90e9ca4d171.png)

通过点击相应的加密按钮对`MYSQL_USERNAME`和`MYSQL_PASSWORD`进行加密：

![](assets/92e642a0-f265-43c3-9dc1-a732bdaf46e3.png)
...
![](assets/3076bb70-b308-4002-9d3e-efed9619dd94.png)
...
![](assets/34a908ea-63f5-4043-8909-39a441edfa46.png)

最后，您可以创建交互式和动态小部件，在 kibana 中构建仪表板，以查看 Lambda 函数事件：

![](assets/8d853e00-8238-48c2-b162-2cb92cddfab2.png)
...
![](assets/451dcc6d-173a-4db9-b34d-e12820aee7f2.png)
