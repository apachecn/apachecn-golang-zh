...
在本章中，我们看到了围棋库中一个非常强大的工具：CGO。这允许 Go 应用程序运行 C 代码，而 C 代码又可以调用 Go 函数。我们看到它需要一个特殊的`import`语句`import "C"`，这是一个伪包，包含所有可供使用的 C 代码。要导出 Go 代码并使其对 C 可用，有一个特殊注释`//export`，它使 Go 函数在 C 名称空间中可用。
