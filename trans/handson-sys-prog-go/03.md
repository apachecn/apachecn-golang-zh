...
Go是一种非常固执己见的语言；有些人喜欢它，有些人讨厌它，主要是因为它的一些设计选择。以下是一些未被广泛接受的功能：
...
Go是垃圾收集；它以计算成本管理自己的内存。编写高效的应用程序需要了解其内存模型和内部结构，以减少垃圾收集器的工作并提高总体性能。
...
在本章中，我们回顾了Go的一些历史及其当前的优缺点。在通过查看包系统和导入如何工作来理解名称空间之后，我们使用基本、复合和用户定义的类型探索了它的类型系统。
