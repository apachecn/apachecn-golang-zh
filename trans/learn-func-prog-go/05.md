...
当我最初编写本章时，我使用了标准的`go get`、`go build`和`go run main.go`命令。随着项目变得越来越复杂（需要更多的第三方依赖），我发现创建 KISS Glide 工具很有帮助。后来，我回到所有的项目，并使用 KISS Glide 工具简化了它们。您可以随意使用任何依赖关系管理工具，并根据自己的喜好构建和运行 Go 应用程序。这是Go最棒的地方之一。只要工具制造商遵守标准的 Go 惯例，比如使用`GOPATH`，而不破坏其他 Go 工具，比如`go test`、`go doc`、`go vet`，一切都好。我更喜欢简单而有力的（亲吻滑翔）。
