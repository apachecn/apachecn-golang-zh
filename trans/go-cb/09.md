...
Go 提供了使并行应用成为可能的原语。Goroutines 允许任何函数变成异步和并发的。通道允许应用设置与 goroutines 的通信。围棋中的一句名言是*不要通过分享记忆来交流；相反，通过与[通信](https://blog.golang.org/share-memory-by-communicating)*来共享内存 https://blog.golang.org/share-memory-by-communicating 。
...
1.  从终端/控制台应用中，创建`chapter9/channels`目录并导航到它。
...
此配方将演示如何使用这些包安全地修改具有各种 goroutine 的映射，并保持可由多个 goroutine 安全访问的全局序号值。它还将展示`Once.Do`方法，该方法可用于确保 Go 应用只执行一次操作，例如读取配置或初始化变量。
...
1.  从终端/控制台应用中，创建`chapter9/atomic`目录并导航到它。
...
对于我们的 map 配方，我们使用了一个`ReadWrite`互斥。这个互斥锁背后的思想是，任意数量的读卡器都可以获得读锁，但只有一个写卡器可以获得写锁。此外，当其他任何人（读卡器或写卡器）拥有锁时，写卡器无法获得锁。这非常有用，因为与标准互斥体相比，读取速度非常快且无阻塞。当我们想要设置数据时，我们使用`Lock()`对象，当我们想要读取数据时，我们使用`RLock()`。最终使用`Unlock()`或`RUnlock()`是非常重要的，这样才能避免应用死锁。延迟`Unlock()`对象可能有用，但可能比手动调用`Unlock()`慢。
...
1.  从终端/控制台应用中，创建`chapter9/context`目录并导航到它。
...
1.  从终端/控制台应用中，创建并导航到`chapter9/state`目录。
...
此配方将创建有状态的工作线程，并演示如何协调和启动多个工作线程，同时在同一通道上处理所有请求。这些工作人员将是加密工作人员，就像在 web 身份验证应用中一样。它们的目的是使用`bcrypt`包散列纯文本字符串，并将文本密码与散列进行比较。
...
1.  从终端/控制台应用中，创建`chapter9/pool`目录并导航到它。
...
1.  从终端/控制台应用中，创建`chapter9/pipeline`目录并导航到它。
