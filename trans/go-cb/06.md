...
1.  从终端/控制台应用中，创建`chapter6/client`目录并导航到它。
...
为 RESTAPI 编写客户机不仅可以帮助您更好地理解所讨论的 API，还可以为将来使用该 API 的所有应用提供有用的工具。这将探索构建客户，并展示一些您可以立即利用的策略。
...
1.  从终端/控制台应用中，创建`chapter6/rest`目录并导航到它。
...
1.  从终端/控制台应用中，创建`chapter6/async`目录并导航到它。
...
1.  从终端/控制台应用中，创建并导航到`chapter6/client`目录。
...
1.  从终端/控制台应用中，创建`chapter6/decorator`目录并导航到它。
...
1.  从终端/控制台应用中，创建并导航到`chapter6/grpc`目录。
