...
Go 应用经常需要使用长期存储。这通常以关系数据库和非关系数据库以及键值存储等形式存在。在使用这些存储应用时，将您的操作包装在一个界面中会有所帮助。本章中的方法将研究各种存储接口，考虑与连接池等事物的并行访问，并查看集成新库的一般提示，这在使用新存储技术时经常出现。
...
2.  打开终端/控制台应用，导航到您的`GOPATH/src`并创建一个项目目录，如
...
1.  从终端/控制台应用中，创建并导航到目录`chapter5/database`中的。
...
1.  从终端/控制台应用中，创建并导航到`chapter5/dbinterface`目录。
...
1.  从终端/控制台应用中，创建并导航到`chapter5/pools`目录。
...
能够控制连接池的深度非常有用。这将允许我们不超载一个数据库，但重要的是要考虑它在超时上下文中意味着什么。如果您同时强制执行一组连接和严格的基于上下文的超时，就像我们在本配方中所做的那样，那么在一些情况下，您将经常在试图建立太多连接的过载应用上请求超时。
...
1.  从终端/控制台应用中，创建并导航到`chapter5/redis`目录。
...
1.  从终端/控制台应用中，创建并导航到`chapter5/mongodb`目录。
...
我们为这些接口添加了一个上下文，以增加额外的灵活性，并允许接口处理超时。将应用逻辑与底层存储分离提供了多种好处，但很难选择正确的位置来划定边界，这将因应用而异。