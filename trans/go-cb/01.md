...
2.  打开终端/控制台应用，导航到您的`GOPATH/src`目录，并创建一个项目目录，如`$GOPATH/src/github.com/yourusername/customrepo`。
...
1.  从终端/控制台应用中，创建一个名为`chapter1/interfaces`的新目录。
2.  导航到该目录。
...
1.  从终端/控制台应用中，创建一个名为`chapter1/bytestrings`的新目录。
2.  导航到此目录。
...
在处理数据时，字节库提供了许多方便的函数。例如，在处理流处理库或方法时，缓冲区比字节数组灵活得多。一旦你创建了一个缓冲区，它就可以用来满足`io.Reader`接口，这样你就可以利用`ioutil`函数来操作数据。对于蒸汽应用，您可能需要使用缓冲区和扫描仪。`bufio`包装在这些情况下很方便。有时，使用数组或切片更适合于较小的数据集，或者当您的计算机上有大量内存时。
...
1.  从终端/控制台应用中，创建一个名为`chapter1/filedirs`的新目录。
2.  导航到此目录。
...
1.  从终端/控制台应用中，创建一个名为`chapter1/csvformat`的新目录。
2.  导航到此目录。
...
1.  从终端/控制台应用中，创建一个名为`chapter1/tempfiles`的新目录。
2.  导航到此目录。
...
模板的另一个应用是网站。当我们想要向客户端呈现服务器端数据时，模板非常适合。首先，Go 模板可能会让人感到困惑。本章将探讨如何使用模板、在目录中收集模板以及如何使用 HTML 模板。
...
1.  从终端/控制台应用中，创建一个名为`chapter1/templates`的新目录。
2.  导航到此目录。
...
模板包提供了您所期望的现代模板库。很容易组合模板，添加应用逻辑，并确保将结果发送到 HTML 和 JavaScript 时的安全性。