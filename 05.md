...
*   它不能用作库。 我们在 `main` 包（策略创建）中编写了关键代码。

    **解决方案**：将从命令行应用程序创建的策略抽象为两个不同的包。

*   这些策略都没有对文件或控制台进行任何日志记录。 我们必须提供一种方法来读取外部用户可以集成到他们的日志记录策略或格式中的一些日志。

    **解决方案**：注入`io.Writer`接口作为依赖项，作为日志接收器。

*  我们的`TextSquare`类始终写入控制台（`io.Writer`接口的实现者），而 `ImageSquare`始终写入文件（`io.Writer`接口的另一个实现者）。 这太耦合了。
...
在这里，我们有一个带有`Get() interface{}`方法的**命令**接口。我们有**a**型和**B**型。其思想是**A**和**B**实现**命令**接口，将自身作为`interface{}`返回。由于现在它们实现了**命令**，因此可以在**命令处理程序**中使用，该处理程序不太关心底层类型。现在**A**和**B**可以通过处理命令或自由存储命令的函数进行传输。但是**B**处理程序可以从任何**命令**处理程序中获取一个对象来“展开”，并获取其**B**内容以及**A**命令处理程序及其`A`内容。
